package entity;

import jakarta.persistence.*;

import java.util.Collection;
import java.util.List;

@Entity
@org.hibernate.annotations.NamedQuery(name = "Specializations.all", query = "from Specializations s")
@org.hibernate.annotations.NamedQuery(name = "Specializations.byName", query = "from Specializations s where upper(s.name) like concat('%', upper(?1), '%')")
public class Specializations {
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Id
    @Column(name = "id")
    private int id;
    @Basic
    @Column(name = "name")
    private String name;
    @OneToMany(mappedBy = "specializationsBySpecialization")
    private List<Entrant> entrantsById = new java.util.ArrayList<>();
    @ManyToOne
    @JoinColumn(name = "faculty", referencedColumnName = "id", nullable = false)
    private Faculties facultiesByFaculty;

    public List<Entrant> getEntrantsById() {
        return entrantsById;
    }

    public void setEntrantsById(List<Entrant> entrantsById) {
        this.entrantsById = entrantsById;
    }

    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;

        Specializations that = (Specializations) o;

        if (id != that.id) return false;
        if (name != null ? !name.equals(that.name) : that.name != null) return false;

        return true;
    }

    @Override
    public int hashCode() {
        int result = id;
        result = 31 * result + (name != null ? name.hashCode() : 0);
        return result;
    }

    public Faculties getFacultiesByFaculty() {
        return facultiesByFaculty;
    }

    public void setFacultiesByFaculty(Faculties facultiesByFaculty) {
        this.facultiesByFaculty = facultiesByFaculty;
    }

    @Override
    public String toString() {
        return  name +
                ", " + facultiesByFaculty;
    }
}
